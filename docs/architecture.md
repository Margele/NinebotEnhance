...
模块只注入九号的主进程（`MirrorModule.mainProcess`，`:pushcore` 等辅助进程不装 hook、不建桥接）。九号进程用 `ServiceBridge` 绑定模块的 `FrameBridgeService` 把模块进程拉起来；系统拒绝绑定（`bindService=false`，ColorOS / HyperOS 等的关联启动管理）时按 1、2、4、8 秒退避重试，并调用 `RootBridgeProvider` 的无副作用 `ping` 尝试让系统以 ContentProvider 方式启动进程，成功即立刻重绑；`ServiceBridge.hyperOs()` 识别 MIUI / HyperOS、ColorOS / OxygenOS / realme、vivo、华为 / 荣耀，绑定被拒时的弹窗据此给出“打开设置”按钮，`AutostartPages` 依次尝试各厂商的自启动页面并回退到应用详情；设置页的“权限设置”按钮（与“授权方式”“统计信息”同一行，所有系统都显示）按组件名启动模块本体 `ui.ModuleActivity`，自启动页从那里的“自启动”一行进。`ui.ModuleActivity` 是唯一的启动器入口，只提供自启动、通知使用权两个系统页面和回到九号的按钮，目的是让 ROM 把模块列进自启动管理并视为用户打开过的应用。

更新检查：`core.UpdateCheck` 解析 tag（`v1.2.3` / `1.2.3`，忽略后缀）、按数值比较版本、把 GitHub 的 release JSON 缩成版本和页面（draft / prerelease 忽略，缺页面时退到 Releases 列表）；`service.UpdateChecker` 在模块进程的后台线程 GET `api.github.com/repos/Margele/NinebotEnhance/releases/latest`，最多 6 小时一次、失败 1 小时后再试，结果存 `update_check` 偏好，`Protocol.UPDATE_CHECK` 只回缓存（含 `checked_at` / `failed_at`）并在过期或 `refresh` 时顺带发起一次查询。九号侧 `DirectCastController` 在非巡航页到前台后 3 / 20 / 60 秒最多问三次，`newer` 且本进程未提示过时，在车辆页（按钮行所在页面）用模块自己的 `ui.UpdateDialog`（`DialogContent` 配色，不用宿主主题的 AlertDialog，否则按钮在九号主题下不可见）弹一次窗：「关闭」和「打开 Release 页」两个按钮；设置页顶部的「更新到 x.y.z」按钮用同一份缓存。关于页的「检查更新」按 `refresh` 强制查一次，每秒回读缓存直到 `checked_at` 或 `failed_at` 晚于点击时刻（最多 10 秒），有新版本弹同一个窗，否则提示已是最新或失败。模块不下载 APK。

`bms.BmsController` 是模块进程里第二条自有 GATT 链路：板子协议由 `BmsSettings.protocol` 选定（默认 `PROTOCOL_AUTO`，`open()` 按 `BmsProtocols.name` 依广播名、`onServicesDiscovered` 按 `BmsProtocols.services` 依暴露服务判定，仍是 AUTO 就退回 DL），`BmsProtocols.create` 返回 `core.BmsProtocol` 实现（`AntBmsProtocol` / `JbdBmsProtocol` / `JkBmsProtocol` / `YyBmsProtocol`，DL 保留自己的加密路径），端点、写类型、要发的命令字节与静默时长都取自 `begin` / `poll` / `followUp` / `silenceMs`；四种普通协议共用 `core.BufferedBmsProtocol` 的接收缓冲——按各自头部扫描、向协议问整帧长度、校验不过就丢弃重同步、在缓冲里停了 1 秒的半帧也丢弃。连接后请求 MTU 247、订阅通知；DL 用本地生成的 secp256k1 密钥对做 FC00 交换（`core.Secp256k1` 纯 BigInteger 实现，共享密钥即乘积的 X 坐标，前 16 字节为 AES 密钥、后 16 字节为 IV），之后按 `BmsSettings.pollMs` 轮询 FC17（`core.DlBmsProtocol`：A5 5A 00 40 请求头、≤4 字节内容字节反转、16 位累加和、整帧 AES-128-CBC 零填充；应答攒满 16 的倍数再解密，按头部长度取整帧），读数进 `BmsState`/`BmsData`，随仪表快照的 `bms` 包（`notification.BmsBundle`）到九号进程；DL 只写 FC00 和 FC17，其余四种只写自己的读命令，都不写板子设置。HUD 的 BMS 卡由 `BmsCardPainter` 画：`BmsCard.Layout` 最多三行、每行若干字段等分宽度，空行隐藏，无新读数时一行“BMS 未连接”，高度经 `SidebarLayout.Sizes.bmsHeight` 进入排版；`WidgetSettings.VOLTAGE_FROM_BMS` / `POWER_FROM_BMS` 让电压、功率卡片和条件在 BMS 读数新鲜时改用它并把曲线采样切到 BMS；`WidgetCondition.BMS_CONNECTED` 是新的“满足条件时”检查项。链路生命周期与大灯相同（`hold(source, ms)` 三个来源）：只在投屏会话、九号界面可见或 BMS 界面持有期间存在，最后一个持有到期即断开、停止轮询并回到 `IDLE`。
