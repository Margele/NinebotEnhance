...
/** Which protection board a device is: by advertised name first, by the services it exposes when the name says nothing. */
public final class BmsProtocols {
    /** The id for an advertised name, or AUTO when the name belongs to none of the boards. */
    public static int name(String label){
        ...
        return BmsSettings.PROTOCOL_AUTO;
    }
    /** The id for the services a device exposes, used when the name is missing; AUTO when they give nothing away. */
    public static int services(Collection<String> uuids){
        if(uuids==null||uuids.isEmpty())return BmsSettings.PROTOCOL_AUTO;
